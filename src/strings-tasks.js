...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (vovels.indexOf(str[i]) >= 0) result += 1;
  }
...
function isPalindrome(str) {
  const str1 = str.toLowerCase().replaceAll(/[^a-zA-Z0-9]/g, '');
...
function findLongestWord(sentence) {
  ...
  const sortedArr = strArr.sort(function sortByLength(a, b) {
...
function getCardId(value) {
  ...
  const suits = [9827, 9830, 9829, 9824];
  const card = value.slice(0, -1);
  const suit = value[value.length - 1].codePointAt(0);

  const firstKey = parseInt(
    Object.keys(cards).find((k) => cards[k] === card),
    10
  );
  const secondKey = parseInt(
    Object.keys(suits).find((k) => suits[k] === suit),
    10
  );
